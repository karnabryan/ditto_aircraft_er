...
for model_name in model_names:
    ...
    if len(df_pred) != len(df_ids):
        raise RuntimeError(
            f"Length mismatch for {model_name}: preds={len(df_pred)} ids={len(df_ids)}. "
    ...
    df_top1_right = top1_per_right_match1(df, score_col="match_confidence")
    df_top1_left = top1_per_left_match1(df, score_col="match_confidence")

    df_top1 = df_top1_left
